...
import { axiosRequestServerRh } from "../../config/request/axios/requestServerToServer";
...
export const createUserExtend = async () => {
    let data = "";
    try {
      const response = await axiosRequestServerRh().post('register', userExtend);
